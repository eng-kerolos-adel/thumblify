    ...
    <div id="pricing" className="px-4 md:px-16 lg:px-24 xl:px-32">
      ...
      <div className="flex flex-wrap items-center justify-center gap-8 mt-20">
        {pricingData.map((plan: IPricing, index: number) => (
          <motion.div
            key={index}
            className={`w-72 text-start border border-pink-950 p-6 pb-16 rounded-xl ${plan.mostPopular ? "bg-pink-950 relative" : "bg-pink-950/30"}`}
          ...
          >
            {plan.mostPopular && (
              <p className="absolute px-3 text-sm -top-3.5 right-8 py-1 bg-pink-400 rounded-full">
                Most Popular
              </p>
            )}
